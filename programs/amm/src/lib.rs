...
pub mod amm {
    ...
    pub fn provide_liquidity(ctx: Context<AddLiquidity>, token_a_amount : u64, token_b_amount : u64) -> Result<()>{
        ...
        require!(ctx.accounts.user_token_a.mint != ctx.accounts.user_token_b.mint, ErrorCode::SimilarTokenError);
        

        //Check that the token user is sending is equal to the token_a we have saved in the pool
        require!(ctx.accounts.user_token_a.mint == pool.token_a, ErrorCode::DifferentTokenError);
        require!(ctx.accounts.user_token_b.mint == pool.token_b, ErrorCode::DifferentTokenError);
        require!(ctx.accounts.lp_mint.key() == pool.lp_mint, ErrorCode::InvalidMintAccount);
        ...
        //If not first time : 
        //Check for the ratio, and required 
        if token_a_amount * reserve_b < token_b_amount * reserve_a{
            //A is limiting
            accepted_a = token_a_amount;
            accepted_b = token_b_amount * pool.reserve_b / pool.reserve_a;
        }else{
            //B is limiting
            accepted_b = token_b_amount;
            accepted_a = token_a_amount * pool.reserve_a / pool.reserve_b;
        }
        ...
        let mut lp_amount = 0;
        if pool.reserve_a == 0 && pool.reserve_b == 0{
            lp_amount = helper::first_lp(token_a_amount, token_b_amount);
        }else{
            lp_amount = helper::calculate_lp(token_a_amount, token_b_amount, pool.reserve_a, pool.reserve_b, total_supply);
        }
        ...
        let cpi_context = CpiContext::new_with_signer(ctx.accounts.token_program.to_account_info(), cpi_account, &[&signer_seeds]);
